...
const { makeElectronOfflineTransport } = require('@sentry/electron/main');
const sentryConfig = {
    sentry,
...
function setupSettingsFlags() {
    ...
    const rawSwitches = settings?.get('chromiumSwitches', []);
    let switches = [];
...
function setupLibOpenH264Switch() {
    ...
    const settings = appSettings.getSettings();
    const openH264Enabled = settings?.get('openH264Enabled', true);
    if (openH264Enabled) {
...
app.on('ready', (_event, launchInfo) => {
    const { registerDesktopConnectivityTests } = require('./desktopConnectivityTests');
    registerDesktopConnectivityTests(session.defaultSession, app.getSystemLocale());
    if (launchInfo?.userInfo?.fallbackDeepLink != null) {
