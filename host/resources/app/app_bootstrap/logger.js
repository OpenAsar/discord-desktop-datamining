...
function initializeLogging(paths) {
    ...
    const rendererLogFile = path_1.default.join(logDir, 'renderer_js.log');
...
function ipcMainRendererLogger(_event, level, message, _lineNo, _sourceId) {
...
function networkDebugLogger() {
    ...
    const log = require('electron-log/main');
    const myLogger = log.create({ logId: 'net_capture' });
    const filePath = path_1.default.join(logDir, `net_capture.log`);
