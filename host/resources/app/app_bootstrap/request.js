...
function wrapPromiseHandlers(url, netInterface, resolve, reject) {
    const wrappedResolve = (response) => {
        handleCallbacks({ url, netInterface, response });
        resolve(response);
    };
    const wrappedReject = (error) => {
        handleCallbacks({ url, netInterface, error });
        reject(error);
    };
...
function nodeRequest({ method, url, headers, qs, timeout, body, stream }) {
    return new Promise((resolve, reject) => {
        const { wrappedResolve, wrappedReject } = wrapPromiseHandlers(url, 'node', resolve, reject);
...
async function electronRequest({ method, url, headers, qs, timeout, body, stream, }) {
    await electron_1.app.whenReady();
    const { net, session } = require('electron');
    const req = net.request({
        method,
        url: `${url}${qs != null ? `?${querystring_1.default.stringify(qs)}` : ''}`,
        redirect: 'follow',
    ...
    return new Promise((resolve, reject) => {
        const { wrappedResolve, wrappedReject } = wrapPromiseHandlers(url, 'electron', resolve, reject);
