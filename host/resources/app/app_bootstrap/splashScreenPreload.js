...
function shouldShowProgress(state) {
    return (typeof state.progress === 'number'
        && (state.status === 'downloading-updates' || state.status === 'installing-updates'));
}
...
function renderSplashHtml(state) {
    const progress = shouldShowProgress(state)
...
function bindManualUpdateEvents() {
    ...
    btn?.addEventListener('click', () => {
        const quit = () => electron_1.ipcRenderer.send('DISCORD_SPLASH_SCREEN_QUIT');
        if (selectedDownload !== 'nope') {
            (0, securityUtils_1.saferShellOpenExternal)(LINUX_DOWNLOAD_URL_BASE + selectedDownload).then(quit, quit);
        }
        else {
            quit();
        }
