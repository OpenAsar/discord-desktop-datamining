...
class DnsQueryBuilder {
    ...
    build() {
        ...
        for (let i = this.labels.length - 1; i >= 0; i--) {
            ...
            const dotLength = 1;
            const newLength = currentLength + dotLength + labelLength;
            if (newLength > 253) {
                break;
            }
...
class DnsReporter {
    ...
    encode(sessionNonce, result) {
        ...
        builder
            .addLabel('idt', sessionNonce, encodeTimestamp(result.timestamp))
            .addLabelIf(this.clientLocale !== undefined, 'loc', this.clientLocale ?? 'unknown')
            .addLabel('chk', result.checkId)
            .addLabel('res', result.success ? 'ok' : 'err')
            .addLabelIf(!result.success, 'err', result.error !== undefined ? encodeError(result.error) : 'unknown')
            .addLabel('dur', encodeDuration(result.durationMs));
...
class ConnectivityTester {
    ...
    constructor(config) {
        ...
        this.sessionNonce = config?.sessionNonce ?? generateNonce();
        this.processDelayMs = config?.processDelayMs ?? 25;
    }
    registerCheck(check) {
