...
const ID_REGEX = /^[0-9]+$/;
const APPLICATION_STORAGE_ENDPOINT = (applicationId, branchId) => `/applications/${applicationId}/branches/${branchId}/storage`;
...
function ensureDispatch() {
    ...
    dispatch = globalThis.window.DesktopNativeUtils.requireModule('discord_dispatch');
}
...
async function generateConfig({ userId, userToken, applicationId, branchId, forceHash, }) {
    ...
    const apiEndpoint = await globalThis.window.DiscordNative.http.getAPIEndpoint();
    return {
        forceHash,
        manifestPath: DefaultCloudSyncConfiguration.STORAGE_MANIFEST(normalizedInstallPath, userId),
        roots: dispatchStorage.roots != null
...
function upgradeConfig(id, config) {
    ...
    const { forceHash, storage: { token: userToken }, replacements: { USERID: userId }, } = config;
...
class CloudSync extends EventEmitter {
    ...
    async sync(optionsOrOldId, oldConfig) {
        ...
        return new Promise((resolve, reject) => this._cloudSync.command(JSON.stringify({ type: 'SYNC', id, config }), makeCallback(resolve, reject)));
    }
}
