import { getClient, _INTERNAL_captureMetric, getCurrentScope } from '@sentry/core';
import { getIPC } from './ipc.js';

function captureMetric(type, name, value, options) {
    const client = getClient();
    _INTERNAL_captureMetric({ type, name, value, unit: options?.unit, attributes: options?.attributes }, {
        scope: options?.scope ?? getCurrentScope(),
        captureSerializedMetric: (_, metric) => getIPC(client).sendMetric(metric),
    });
}
/**
