...
function captureMetric(type, name, value, options) {
    ...
    core._INTERNAL_captureMetric({ type, name, value, unit: options?.unit, attributes: options?.attributes }, {
        scope: options?.scope ?? core.getCurrentScope(),
        captureSerializedMetric: (_, metric) => ipc.getIPC(client).sendMetric(metric),
    });
}
/**
